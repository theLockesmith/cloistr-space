      ...
      {isOpen && (
        <div className="absolute right-0 z-10 mt-1 w-36 rounded-lg border border-cloistr-light/10 bg-cloistr-dark py-1 shadow-lg">
          ...
          <button
            ...
            className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-cloistr-error hover:bg-cloistr-error/10"
          >
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <span
        className={`rounded px-2 py-0.5 text-xs ${
          task.priority === 'high'
            ...
            : task.priority === 'low'
              ? 'bg-cloistr-text-dim/10 text-cloistr-text-dim'
...
function WidgetError({ message }: { message: string }) {
  return (
    <div className="rounded-lg border border-cloistr-error/20 bg-cloistr-error/5 p-4 text-center">
      <p className="text-sm text-cloistr-error">{message}</p>
    </div>
  );
}
